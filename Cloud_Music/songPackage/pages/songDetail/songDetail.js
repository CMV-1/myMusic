...
Page({
  ...
  data: {
    ...
    showList: 58, // 显示歌单列表
    isLyricsShow: false, // 是否显示歌词
    lyrics: [['Tip', '正在加载歌词']], // 存放歌词
    lyricsIndex: 0, // 当前歌词位置索引
    isLike: false, // 是否喜欢当前音乐
    displayMusicList: [],
    scrollPosition: 0, // 音乐列表滑动到的位置
    // 用于切歌时重置动画, true代表加载完成，false代表未完成，
    // 可以通过加载存在时间间隔达到删除并重置动画类名从而重置动画的效果
    switchMusicAnimationState: true,
    discSwiper: [[], [], []], // 碟子的swiper
    isNeedlePlay: false, // 指针是否是播放状态
    discSwiperIndex: 0  // 碟子当前swiper-item的索引
  },
  ...
  onLoad: async function (options) {
    ...
    wx.getSystemInfo({
      success: e => {
        let barheight = e.statusBarHeight; //状态栏高度
        this.setData({
          barHeight: barheight
        })
      }
    ...
    // 获取当前播放歌曲索引
    this.getPlayingMusicIndex(options.musicId)
    // 获取musicInfo
    await this.getMusicInfo(options.musicId)
  ...
  async like() {
    ...
    // 喜欢该音乐
    request('/like', { id: musicId, like: this.data.isLike }, 1, 'post')
  ...
  async getLikeList() {
    ...
    if (app.globalData.likeList.length == 0) {
      result = await request('/likelist', { uid: userId }, 1)
      // likeList = result.ids;
      // 存入全局变量
      app.globalData.likeList = likeList
    } else {
  ...
  toComments() {
    wx.navigateTo({
      url: '/pages/songComment/songComment?musicid=' + musicId
    })
  },
